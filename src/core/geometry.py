...
class EdgeSample:
    edge_idx: int
    edge_position: float # коэффициент t (0=start, 1=end): p = a + t(b - a)
...
def get_point_on_segment(polygon: Shape, sample: EdgeSample) -> Point:
    a, b = polygon.get_edge(sample.edge_idx)
    return a + sample.edge_position * (b - a)


def point_to_centers_distance(polygon: Shape, edge_idx: int, t: float, centers: List[Point]) -> float:
